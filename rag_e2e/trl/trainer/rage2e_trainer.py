...
class ArceeRagTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.config.backward_batch_size = (
            self.config.mini_batch_size * self.config.gradient_accumulation_steps
        )
    ...
    def marginalized_log_probs(
        ...
        logits = self.get_logits(self.model, prompt_tensors, prompt_attention)

        logprobs_logits = logprobs_from_logits(
            logits[:, :-1, :], prompt_tensors[:, 1:], gather=False
        ).view(logits.shape[0], n_docs, -1, logits.size(-1))
    ...
    def compute_marginalized_loss(
        ...
        logits = self.get_logits(self.model, input_tensors, input_attention)
        return self.compute_marginalized_loss_from_logits(
            logits, input_tensors, scores, query_token_length
        )
