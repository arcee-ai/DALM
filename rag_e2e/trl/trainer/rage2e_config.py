...
def autotag() -> str:
    ...
    try:
        ...
        if git_branch != "main":
            ...
            prs = requests.get(
                f"https://api.github.com/search/issues?q=repo:lvwerra/trl+is:pr+{git_commit}"
            )
...
class RagE2EConfig(object):
    ...
    backward_batch_size: Optional[int] = field(
        default=1,
        metadata={"help": "Number of samples optimized in an `optimizer.step()` call"},
    )
                ...
                if len(wandb_tag) > 0:
                    if len(existing_wandb_tag) > 0:
                        os.environ["WANDB_TAGS"] = ",".join(
                            [existing_wandb_tag, wandb_tag]
                        )
                    else:
                        os.environ["WANDB_TAGS"] = wandb_tag
                    logging.info(
                        f"the following tags will be used for wandb logging: {os.environ['WANDB_TAGS']}"
                    )
            except ImportError:
                raise ImportError(
